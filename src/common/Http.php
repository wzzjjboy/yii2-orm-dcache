...
class Http
{
    ...
    protected function request($url, $method = self::GET, $params = array(), $options = array())
    {
        ...
        if(isset($options['timeOut']) && intval($options['timeOut'])>0) {
            curl_setopt($this->curl, CURLOPT_TIMEOUT, intval($options['timeOut']));
        ...
        if (isset($options['files']) && count($options['files'])) {
            ...
            if (isset($options['json'])) {
                is_array($params) && $params = JSON::encode($params, JSON_UNESCAPED_UNICODE+JSON_UNESCAPED_SLASHES);
        ...
        try{
            ...
            if(CURLE_OPERATION_TIMEDOUT  == $exception->getCode()){
                usleep(50000); //休眠50毫秒
                $response = $this->doCurl();
            } else {
                throw $exception;
